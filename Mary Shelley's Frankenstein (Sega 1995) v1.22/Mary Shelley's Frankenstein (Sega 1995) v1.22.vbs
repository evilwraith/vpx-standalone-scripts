...
Sub sw31e_Hit()  : KickScoop ActiveBall, 110, 25 : sw31e.Enabled = False : End Sub
...
Sub sw31_Timer()
	' top scoop
	SaucerAction sw31Step, Nothing, sw31Ball, 31, sw31, 292, 150+Rnd()*20
	sw31Step = sw31Step + 1
End Sub
Sub sw32_Timer()
	' bottom scoop
...
Sub kFrankLeftHand_unHit:handfull = 0:End Sub
...
Sub trUpperRampStart_Hit()
	SlowDownBall ActiveBall
End Sub
...
Sub trLowerRampBeforeStart_Hit()
	isBallOnRamp = False
End Sub
Sub trLowerRampStart_Hit()
	isBallOnRamp = True
End Sub
Sub trLowerRampEnd_Hit()
	isBallOnRamp = False
End Sub
...
Sub RollingSoundTimer_Timer()
    ...
    For b = 0 to UBound(BOT)
		...
		If BOT(b).VelZ < -2 And BOT(b).Z < 55 And BOT(b).Z > 32 And Not isBallOnRamp Then
			PlaySound "fx_balldrop" & b, 0, ABS(BOT(b).VelZ)/17*5, AudioPan(BOT(b)), 0, Pitch(BOT(b)), 1, 0, AudioFade(BOT(b))
		End If
